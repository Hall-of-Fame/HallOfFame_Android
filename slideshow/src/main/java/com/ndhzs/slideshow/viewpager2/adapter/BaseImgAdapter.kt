...
abstract class BaseImgAdapter<T> : BaseViewAdapter<ShapeableImageView>() {
    ...
    override fun onBindViewHolder(
        ...
        val falsePosition = BasePageChangeCallBack.getFalsePosition(mIsCirculate, position, beans.size)
        if (array.indexOfKey(falsePosition) >= 0) {
            ...
            when (conditionWithListener.condition) {
                Refresh.Condition.COEXIST -> {
                    refactor(beans[falsePosition], view, holder, falsePosition)
     ...
     */
    @Deprecated("禁止自己调用! ")
    internal fun <E> refreshData(beans: List<E>) {
        outerData = beans as List<T>
        this.beans.clear()
    ...
    internal fun openCirculateEnabled() {
        if (!mIsCirculate) {
            if (beans.size > 1) {
                mIsCirculate = true
                size = if (beans.size <= 10) 30 else beans.size * 3
            }
        }
    }
